...
class PluginRegistry:
    ...
    def _load_plugin_module(self, module_name: str) -> PluginLoadResult:
        ...
        try:
            ...
            return PluginLoadResult(
                success=True,
                manifest=self._manifests.get(plugin_cls.metadata.id),
            )
    ...
    def _register_plugin_class(self, plugin_cls: type) -> None:
        """
    ...
    async def reload_plugin(self, plugin_id: str) -> PluginLoadResult:
        ...
        module_name = plugin_id.split("-")[0] if "-" in plugin_id else plugin_id
    ...
    def _get_plugin_class(self, manifest: PluginManifest) -> type | None:
        ...
        # Try to find in registered manifests
        for ep in PluginEntryPoint:
            if plugin_id in self._entry_points.get(ep, []):
                # The class should be registered via __init_subclass__
                # We need to re-import to get the class
                module_name = plugin_id.split("-")[0] if "-" in plugin_id else plugin_id
                try:
                    module = importlib.import_module(module_name)
                    for attr_name in dir(module):
                        ...
                        if (
                            ...
                            and hasattr(attr, "metadata")
                            and attr.metadata.id == plugin_id
                        ):
                            return attr
                except ImportError:
                    pass
