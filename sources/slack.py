...
class SlackSource(SourceAdapter):
    ...
    async def _ack(self, client: SocketModeClient, request: SocketModeRequest) -> None:
        response = SocketModeResponse(envelope_id=request.envelope_id)
        maybe_awaitable = client.send_socket_mode_response(response)
    ...
    async def _handle_socket_mode_request(self, client: SocketModeClient, request: SocketModeRequest):
        ...
        try:
            if request.type == "events_api":
                ...
                await self._ack(client, request)
                ...
                event = EventEnvelope(
                    source=EventSource(type="slack", id=channel_id, name="slack"),
                    sender=EventSender(
                        id=user.get("id", ""),
                        name=user.get("username", user.get("name", user.get("id", ""))),
                        trust_level="user",
                    ),
                    event=EventMeta(
